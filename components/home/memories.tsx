...
export function Memories() {
  ...
  const years = useMemo(
    () => ["All", ...Array.from(new Set(MEMORIES.map((m) => m.year))).sort()],
    [],
  );
  ...
  return (
    <section
      id="memories"
      className="flex h-screen shrink-0 snap-start flex-col justify-center px-6 sm:px-10"
      style={{ background: HOME.bgSurface }}
    >
      <div className="mx-auto w-full max-w-6xl">
        ...
        <div className="relative mt-10">
          <button
            ...
            onClick={() => scrollBy(-1)}
            className="absolute top-1/2 -left-4 z-10 hidden h-10 w-10 -translate-y-1/2 place-items-center rounded-full border transition-colors sm:grid"
            style={{ background: HOME.bgCard, borderColor: HOME.border, color: HOME.textSecondary }}
          >
          ...
          <button
            ...
            onClick={() => scrollBy(1)}
            className="absolute top-1/2 -right-4 z-10 hidden h-10 w-10 -translate-y-1/2 place-items-center rounded-full border transition-colors sm:grid"
            style={{ background: HOME.bgCard, borderColor: HOME.border, color: HOME.textSecondary }}
          >
          ...
          >
            {filtered.map((m) => (
              <div
                key={m.file}
                className={cn(
                  "relative flex shrink-0 flex-col items-center justify-center gap-3 overflow-hidden rounded-md border",
                  "h-45 w-65 sm:h-60 sm:w-85",
                )}
              ...
              >
                ...
                <div className="text-center">
                  <p
                    style={{ fontFamily: "var(--font-dm-serif)", fontSize: 20, color: HOME.textPrimary }}
                  >
                    {m.occasion}
                  </p>
                  <p
