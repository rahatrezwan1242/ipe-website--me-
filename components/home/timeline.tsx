...
function PhotoCell({ label }: { label: string }) {
  return (
    <div
      className="flex aspect-4/3 items-center justify-center overflow-hidden rounded-sm border"
      style={{ background: HOME.bgSurface, borderColor: HOME.border }}
    >
...
function EventCard({ event }: { event: Event }) {
  return (
    <div
      ...
      style={{ borderColor: HOME.border }}
    >
      <div
        className="px-5 py-3.5 text-center"
        style={{ background: `color-mix(in oklab, ${HOME.accentTeal} 18%, transparent)`, borderBottom: `1px solid ${HOME.border}` }}
      >
        <span
          style={{ fontFamily: "var(--font-dm-serif)", fontSize: 22, color: HOME.textPrimary, letterSpacing: "0.01em" }}
        >
          {event.title}
        </span>
        {event.description && (
      ...
      {event.photos && (
        <div
          ...
          style={{
            ...
            gridTemplateColumns: event.photos.length === 1 ? "1fr" : "1fr 1fr",
          }}
        >
          {event.photos.map((label, i) => (
            <PhotoCell key={i} label={label} />
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <div className="relative grid h-7 w-7 shrink-0 place-items-center">
      <span
        ...
        style={{ border: `1.5px dashed ${HOME.accentTeal}`, opacity: 0.6 }}
      />
      <span
        className="h-3 w-3 rounded-full"
        style={{ background: HOME.accentTeal, border: `2px solid ${HOME.bgBase}` }}
      />
    </div>
  );
}
      ...
      <div className="flex gap-4 md:hidden">
        ...
        <div className={cn("min-w-0 flex-1", reveal)}>
          <span
            ...
            style={{ fontFamily: "var(--font-jetbrains)", color: HOME.textSecondary, opacity: 0.85 }}
          >
            {event.date}
          </span>
      ...
      <div className="hidden grid-cols-[1fr_56px_1fr] items-start md:grid">
        {isLeft ? (
          ...
          <div className="flex justify-end pt-3">
            <DateLabel date={event.date} align="right" />
          </div>
        )}
        ...
        {isLeft ? (
          <div className="flex justify-start pt-3">
            <DateLabel date={event.date} align="left" />
          </div>
        ) : (
    ...
    <section id="timeline" className="px-6 py-24 sm:px-10" style={{ background: HOME.bgBase }}>
      <div className="mx-auto max-w-6xl">
        ...
        <div className="relative">
          <div
            aria-hidden
            className="absolute top-0 bottom-0 hidden w-0.5 -translate-x-1/2 md:left-1/2 md:block"
            style={{ background: SPINE_GRADIENT }}
          />
          <div
            aria-hidden
            className="absolute top-0 bottom-0 left-3.25 w-0.5 md:hidden"
            style={{ background: SPINE_GRADIENT }}
          />
