...
const TOTAL_CREDITS = curriculum.semesters.reduce(
  (sum, s) => sum + s.total_credits,
  0,
);
    ...
    <main className="mx-auto min-h-screen max-w-3xl bg-white px-6 py-10 text-black print:max-w-none print:p-0">
      ...
      <header className="mb-8">
        <p className="text-xs uppercase tracking-wider text-neutral-500">
          Islamic University of Technology &middot; Batch 34
        </p>
        <h1 className="mt-1 text-2xl font-semibold">{curriculum.program}</h1>
        <p className="mt-1 text-sm text-neutral-600">
          8 semesters &middot; {TOTAL_CREDITS.toFixed(2)} total credit hours
          &middot; {curriculum.optional_credits.toFixed(0)} elective credits
        </p>
        <p className="mt-3 text-xs leading-relaxed text-neutral-500">
          Unofficial, student-compiled summary based on the department&apos;s
          program structure. Course offerings and credit hours can be revised
          &mdash; verify against the current official syllabus for your
          admission year before making academic decisions.
        </p>
      </header>
      ...
      {curriculum.semesters.map((sem, i) => (
        <section key={sem.name} className="mb-8 break-inside-avoid-page">
          <h2 className="mb-2 flex items-baseline justify-between border-b-2 border-black pb-1 text-base font-semibold">
            <span>
              Semester {i + 1} &mdash; {sem.name}
            </span>
            <span className="text-sm font-normal text-neutral-600">
              {sem.total_credits.toFixed(2)} credits
            </span>
          </h2>
            ...
            <tbody>
              {sem.courses.map((c) => (
                <tr key={c.code} className="align-top">
                  <td className="border-b border-neutral-100 py-1 pr-2 font-mono text-[11px]">
                    {c.code}
                  </td>
                  <td className="border-b border-neutral-100 py-1 pr-2">{c.title}</td>
                  <td className="border-b border-neutral-100 py-1 pr-2 text-neutral-600">
                    {c.course_type ?? "—"}
                  </td>
                  <td className="border-b border-neutral-100 py-1 text-right">
                    {c.credits.toFixed(2)}
                  </td>
                </tr>
              ))}
      ...
      <footer className="mt-10 border-t border-neutral-200 pt-4 text-xs text-neutral-500">
        IUT &middot; Industrial &amp; Production Engineering &mdash; Batch 34.
        Generated from the curriculum data used by this site&apos;s GPA
        calculator.
      </footer>
    </main>
  );
