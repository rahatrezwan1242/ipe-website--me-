...
A Next.js 16 (App Router, Turbopack) site for IUT's Department of Industrial & Production Engineering (IPE). Batch name/number is not yet decided — don't invent one; the homepage currently uses "IUT IPE" as a neutral placeholder brand. Three pages: `/` (home placeholder), `/curriculum` (placeholder), and `/calculator` (the actual product — semester GPA entry, CGPA overview, AI advisor). The repo lives in this directory (`web/`) and deploys to Netlify on every push to `main`. The legacy vanilla-HTML calculator at `../Project/` is unrelated and not part of this deploy.
